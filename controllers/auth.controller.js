...
const { emailIsConfirmed } = require("../util/validation");
...
async function signup(req, res, next) {
  ...
  if (
    ...
    !userDetailsAreValid.emailIsConfirmed(
      req.body.email,
  ...
  try {
    ...
    if (existsAlready) {
      sessionFlash.flashDataToSession(
        req,
        {
          error: "User exists already !! Try logging in instead",
          ...enteredData,
        },
        () => {
          res.redirect("/signup");
...
async function login(req, res) {
  ...
  const sessionErrorData = {
    errorMessage: "Invalid credentials !! Please recheck your entries",
    email: user.email,
